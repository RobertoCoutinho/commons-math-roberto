...
public class FiniteDifferencesDifferentiatorTest {
    ...
    public void testConstant() {
        ...
        for (double x = -10; x < 10; x += 0.1) {
            ...
            double expecteDerivative = 42.0;
            double defaultDelta = 1.0e-15;
            Assert.assertEquals(expecteDerivative, y.getValue(), defaultDelta);
            expecteDerivative = 0.0;
            Assert.assertEquals(expecteDerivative, y.getPartialDerivative(1), defaultDelta);
            Assert.assertEquals(expecteDerivative, y.getPartialDerivative(2), defaultDelta);
        }
    }
    ...
    public void testLinear() {
        ...
        for (double x = -10; x < 10; x += 0.1) {
            DerivativeStructure y = f.value(new DerivativeStructure(1, 2, 0, x));
            double expecteDerivative = (2 - 3 * x - y.getValue());
            double actual = 2 - 3 * x;
            double delta = 2.0e-15;
            Assert.assertEquals("" + expecteDerivative, actual, y.getValue(), delta);
            expecteDerivative = -3.0;
            delta = 4.0e-13;
            Assert.assertEquals(expecteDerivative, y.getPartialDerivative(1), delta);
    ...
    public void testStepSizeUnstability() {
        ...
        UnivariateDifferentiableFunction badStep = new FiniteDifferencesDifferentiator(7, 1.0e-6)
                .differentiate(quintic);
        ...
        // the 0.25 step size is good for finite differences in the quintic on this
        // abscissa range for 7 points
        // the errors are fair
        final double[] expectedGood = new double[] {
                7.276e-12, 7.276e-11, 9.968e-10, 3.092e-9, 5.432e-8, 8.196e-8, 1.818e-6
        };

        // the 1.0e-6 step size is far too small for finite differences in the quintic
        // on this abscissa range for 7 points
        // the errors are huge!
    ...
    public void testWrongOrder() {
        UnivariateDifferentiableFunction f = new FiniteDifferencesDifferentiator(3, 0.01)
    ...
    public void testWrongOrderMatrix() {
        UnivariateDifferentiableMatrixFunction f = new FiniteDifferencesDifferentiator(3, 0.01)
                .differentiate(new UnivariateMatrixFunction() {
                    @Override
                    public double[][] value(double x) {
                        // this exception should not be thrown because wrong order
                        // should be detected before function call
    ...
    public void testBounds() {
        ...
        UnivariateDifferentiableFunction properlyBounded = new FiniteDifferencesDifferentiator(3, 0.1, 0.0, 1.0)
                .differentiate(f);
        DerivativeStructure tLow = new DerivativeStructure(1, 1, 0, 0.05);
        DerivativeStructure tHigh = new DerivativeStructure(1, 1, 0, 0.95);

        try {
            // here, we did not set the bounds, so the differences are evaluated out of
            // domain
    ...
    public void testBoundedSqrt() {
        ...
        UnivariateFunctionDifferentiator differentiator = new FiniteDifferencesDifferentiator(9, 1.0 / 32, 0.0,
        ...
        // we are able to compute derivative near 0, but the accuracy is much poorer
        // there

...
public class SparseGradientTest extends ExtendedFieldElementAbstractTest<SparseGradient> {
        ...
        for (int maxOrder = 1; maxOrder < 5; ++maxOrder) {
            ...
            for (SparseGradient sg : list) {
                ...
                for (double a : new double[] { 0.0, 0.1, 1.0, 2.0, 5.0 }) {
                    SparseGradient reference = (a == 0) ? x.getField().getZero()
            ...
            // negative base: -1^x can be evaluated for integers only, so value is sometimes
            // OK, derivatives are always NaN
        ...
        for (double x = 0; x < 2; x += 0.2) {
            ...
            for (double y = 0; y < 2; y += 0.2) {
                ...
                for (double z = 0; z > -2; z -= 0.2) {
                    ...
                    SparseGradient sg = sgZ.linearCombination(1, sgX,
                            5, sgX.multiply(sgY),
                            -2, sgZ,
    ...
    public void testHypotSpecial() {
        ...
        Assert.assertTrue(
                Double.isInfinite(SparseGradient.hypot(SparseGradient.createVariable(0, Double.POSITIVE_INFINITY),
                        SparseGradient.createVariable(0, +3.0e250)).getValue()));
    ...
    public void testAtan2SpecialCases() {
        ...
        SparseGradient pp = SparseGradient.atan2(SparseGradient.createVariable(1, +0.0),
                SparseGradient.createVariable(1, +0.0));
        Assert.assertEquals(0, pp.getValue(), 1.0e-15);
        Assert.assertEquals(+1, JdkMath.copySign(1, pp.getValue()), 1.0e-15);

        SparseGradient pn = SparseGradient.atan2(SparseGradient.createVariable(1, +0.0),
                SparseGradient.createVariable(1, -0.0));
        Assert.assertEquals(JdkMath.PI, pn.getValue(), 1.0e-15);

        SparseGradient np = SparseGradient.atan2(SparseGradient.createVariable(1, -0.0),
                SparseGradient.createVariable(1, +0.0));
        Assert.assertEquals(0, np.getValue(), 1.0e-15);
        Assert.assertEquals(-1, JdkMath.copySign(1, np.getValue()), 1.0e-15);

        SparseGradient nn = SparseGradient.atan2(SparseGradient.createVariable(1, -0.0),
                SparseGradient.createVariable(1, -0.0));
    ...
    public void testTanhDefinition() {
        for (double x = 0.1; x < 1.2; x += 0.001) {
            SparseGradient sgX = SparseGradient.createVariable(0, x);
            SparseGradient tanh1 = sgX.exp().subtract(sgX.exp().reciprocal())
                    .divide(sgX.exp().add(sgX.exp().reciprocal()));
